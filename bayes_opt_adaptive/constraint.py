...
class ConstraintModel():
    ...
    def __init__(self, fun, lb, ub, relax_margin=0, random_state=None):
        self.fun = fun
    ...
    def predict(self, X):
        ...
        if len(self._model) == 1:
            ...
            for j, gp in enumerate(self._model):
                ...
                result = result + (p_upper - p_lower)/len(self._model)
    ...
    def allowed(self, constraint_values):
        """
        Checks whether `constraint_values` are within 1% of the specified limits.
        """
        lb_allowance = self._lb * self.relax_margin #0.001  # 0.1% margin of lower limit  
        ub_allowance = self._ub * self.relax_margin #0.001  # 0.1% margin of upper limit

        if self._lb.size == 1:
            return (np.less_equal(self._lb - lb_allowance, constraint_values)
                    & np.less_equal(constraint_values, self._ub + ub_allowance))

        return (np.all(constraint_values <= self._ub + ub_allowance, axis=-1)
                & np.all(constraint_values >= self._lb - lb_allowance, axis=-1))
