...
class TargetSpace(object):
    ...
    def register(self, params, target, constraint_value=None):
        ...
        if x in self:
            self._allow_duplicate_points = True
        ...
        if isinstance(constraint_value, float):
            constraint_value = [constraint_value]  # Convert to a list
        for value, lower_bound, upper_bound in zip(constraint_value, self._constraint.lb, self._constraint.ub):
            if value >= lower_bound * 0.99 and value <= upper_bound * 1.01:
                if all(target > tar for tar in self._target):
                    constraint_satisfied = True
        if constraint_satisfied:
            # If the condition is met, add x to the list
            self.seeds3.append(x)
            if len(self.seeds3) > 6:
               self.seeds3.remove(min(self.seeds3))
    ...
    def probe(self, params):
        ...
        if isinstance(params, dict):
            ...
            if self._constraint is None:
                ...
                target,constraint_value = self.target_func(**params)
            ...
            print('Call the target function in parallel, the number of parallel calls is:',len(params))
            process_num = len(params)
            with Pool(process_num) as p:
                Results = p.starmap(self._evaluate_single_point, [(param,) for param in params])
            # Update shared state
            for result in Results:
                self._update_shared_state(result)
            return Results

    def _update_shared_state(self, result):
        """
        Callback function to update shared state.

        Parameters
        ----------
        result : tuple
            Tuple containing parameters and target function value.
        """

        x = result[0]
        target = result[1]
        if len(result) == 3:
            constraint_value = result[2]
            self.register(x, target, constraint_value)
        else:
            self.register(x, target)

    def _evaluate_single_point(self, params):
        """
        Helper function to evaluate a single point in the target function.
        ...
        """
        x = self._as_array(params)
        params_dict = dict(zip(self._keys, x))
        if self._constraint is None:
            target = self.target_func(**params_dict)
            return x,target
        else:
            target, constraint_value = self.target_func(**params_dict)
            return x,target, constraint_value
